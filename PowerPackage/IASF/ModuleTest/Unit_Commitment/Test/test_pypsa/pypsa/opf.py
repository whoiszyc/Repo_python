...
def define_linear_objective(network,snapshots):
    ...
    model = network.model  # pyomo model (ZYC)
    ...
    objective = LExpression() # imported object from opt (ZYC)
...
def network_lopf_build_model(network, snapshots=None, skip_pre=False,
    ...
    # Define the pyomo environment (ZYC)
    print("Define the pyomo environment: network.model created (Develop mode installation test by ZYC)") # (ZYC)
